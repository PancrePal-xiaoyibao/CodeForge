...
> **宿主机级初始化转交**：若用户请求的是新机器 onboarding、`/ai-spec init`、或生成宿主机全局注入文档（`~/.claude/CLAUDE.md` / `~/AGENTS.md`），转交 `dev-host-init` skill 执行（环境探测 + 开发环境缺口提醒 + codebase-memory-mcp 部署提醒 + 注入文档生成）；本 skill 专注项目级需求→SPEC。
