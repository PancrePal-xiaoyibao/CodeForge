...
| 关系 | Skill | 场景 |
|------|-------|------|
| 可调用 | code-debugger | 自动开发遇到错误时 |
| 可调用 | code-review | 每轮实现后作为质量门审查 |
| 可调用 | codebase-context | 开发前理解架构约束 |
| 可调用 | nodejs-npm-auto-release | 开发完成需要发布 |
| 被调用 | ai-spec | 主调度路由到自动开发 |
| 被调用 | prd | PRD 完成后启动开发循环 |
| 被调用 | api-first | API 设计完成进入实现 |
