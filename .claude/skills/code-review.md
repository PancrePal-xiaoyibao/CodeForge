...
作为审查的一环，检查 diff 中测试闭环留下的"屎"，执行测试产物卫生铁律（完整规则见 `code-debugger` T0–T3）。以下各项除非有明确正当理由，否则至少记为 **Medium**：

- **测试产物泄漏进仓库**：测试框架临时输出、fixture 残留、未被 .gitignore 覆盖的生成物、测试临时 db/缓存/截图被提交。
- **代码层老鼠屎**：`print`/`console.log`/调试输出、`debugger`/breakpoint 语句、注释掉的测试代码块、临时 TODO/FIXME、硬编码测试数据/魔法数字、未清理的 mock 注入、被注释掉的旧实现。

泄漏的**运行数据**记为 **High**（删除风险），并确认纯测试产物已被 T1 清理而非提交。审查期间不自动删除——只通报并路由到 `code-debugger` 清理。
