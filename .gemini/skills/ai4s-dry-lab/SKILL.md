...
> **CodeForge 跨仓库 Skill 注记**：本联动框架引用的 skill 分两类——
> - **CodeForge 内置**（本仓库三镜像已有）：`deep-research`
> - **跨仓库生态 skill**（宿主机全局或独立仓库维护，CodeForge 不内置）：`paper-reader`、`scrna-bindlab-full-workflow`、`scrna-celltype-annotation`、`midas`、`thesis-writing-mentor`、`sci-journal-submission-expert`、`paper-submission-manager`、`pubmed-linker`、`academic-orchestrator`
>
> 调用跨仓库 skill 时，若目标环境未安装对应 skill，本框架降级为「直接用 MCP（PubMed/OpenAlex）+ 手工撰写」完成等效工作，不阻塞主循环。联动架构作为设计参考保留；实际可用 skill 以当前环境 `/discover-skill` 探测为准。
...
| 被调用 | academic-orchestrator | 实验分析阶段 |
