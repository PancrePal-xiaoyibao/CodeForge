...
- ❌ **不做综合判断就给用户甩半成品** — 不要把"查清楚了根因 + 一张表"当交付，然后问用户"要不要处理"。**该自己基于事实下判断的，自己下**：真故障报修复方案、设计行为说"无需动"、遗留噪音给清理动作并说明依据，让用户做选择题而非问答题。
- ❌ **为完成而完成、忽视产出价值** — 数量不是产出。100 个工具逐个测了一遍但漏了关键链路（入口/出口），不如 10 个工具测到位。宁可少而准，不要多而假。**假绿的全面 = 比不测更危险**，因为它让所有人误判已验收。

**判定速查**：动手前能不能一句话说清"为什么这么做、不这么做会怎样、这么做对谁有什么影响"？说不清 → 停下，先想。产出给用户前，问自己：这是**结论 + 依据 + 可执行下一步**，还是只是**信息罗列 + 甩问题**？前者合格，后者不合格。

**这条铁律凌驾于本文件其他流程性规则（Codebase 索引优先、多 Agent 协作、测试产物卫生、开发审美等）之上**——那些都是手段，**动脑子综合判断才是目的**。手段服务于目的，不能反过来为了走流程而牺牲判断。
