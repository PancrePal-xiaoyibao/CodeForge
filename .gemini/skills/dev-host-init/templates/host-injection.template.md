...
## 2. 事实源与代码发现（第一级·codebase 图谱优先）

> 核心思想：**图谱先行，索引保障，禁止跳层直读**。图谱是唯一通行证——先搜索；搜不到说明索引可能不新，重新 embedding 让索引追上代码，再继续用图谱，不到万不得已不 read。

### 2.1 图谱先行（第一步·必做）

- 进入项目先读适用的项目规则；代码结构探索**先查 codebase-memory 索引**，未索引优先建 full 索引。
...
### 2.2 搜不到 → 索引保障（关键：重新 embedding，禁止跳读）

- **图谱空结果先检查：项目名、参数、索引状态。** 参数无误仍搜不到 → **怀疑索引过期（embedding 未跟上最新代码），不是代码不存在。**
- **此时必须重新 embedding 更新图谱**：未索引 → `index_repository --mode full`；索引可能过期/代码比索引新（含未提交变更、本次会话新写代码、他人刚 push）→ 先 `detect_changes` 检查影响，或直接 `index_repository` 重新索引。
- **重新索引完成后回到图谱再查一次**：这次通常就能命中——因为索引追上了代码。**❌ 绝对禁止"搜不到 → 跳过图谱 → 直接 read"**：跳读说明索引没跟上代码，read 到的是"旧图谱没覆盖的新代码"，等于把索引干废了。
- 索引重建后仍确认搜不到（真不存在）：此时才允许 fallback——简述原因后用 git diff、rg 和精准源码读取继续。
...
## 二、强制执行顺序（核心闭环：图谱先行 → 索引保障 → 回图谱 → 兜底才直读）

```
Level 1 — 图谱查询（必须优先）
   调用：trace_path / search_graph / query_graph / get_architecture
   ┌─ 命中 → 继续
   └─ 空结果 → 先查参数（拼写/项目名/label/过滤条件）正确吗？
              ├─ 参数不对 → 修正后回到 Level 1 重查
              └─ 参数正确仍空 → 怀疑【索引过期】(embedding 未跟上新代码)，进 Level 2

Level 2 — 索引保障（关键：重新 embedding，禁止跳过图谱直读）
   未索引 → index_repository --mode full 建索引
   索引可能过期（代码比索引新：未提交变更/本次新写/他人刚 push）
         → detect_changes 检查影响，或直接 index_repository 重新 embedding
   ⚠️ 完成后【必须回到 Level 1 再查一次】——索引追上代码后通常就命中了
   ❌ 禁止"搜不到 → 跳过图谱 → 直接 read"（index 干废的根源）

Level 3 — 逐文件阅读（final fallback，仅在重索引后仍搜不到时）
   先 index_status / 图谱查询确认已索引
   然后 search_code files 模式定位路径
   最后 read(path) 读具体文件（简述跳读原因）
```

### 🔒 硬约束

- **不允许**直接从 Level 1 跳到 Level 3（中间必过 Level 2 索引保障 + 回图谱重查）
...
6. **未索引的文件**：如果 `search_code(files)` / `index_status` 说未索引，需要先索引项目，重新 embedding 后回到图谱重查，不要直接读取大段源码
...
2. **第一级·codebase 索引优先**：一切代码探索先走知识图谱（search_graph / trace_path / get_code_snippet / query_graph / get_architecture），禁止跳层直接 read 代码文件。两个第一级都该做时同时执行：图谱先行摸结构 → 判断随后定怎么改。
   - **搜不到 → 索引保障**：图谱空结果先查参数（拼写/项目名/label）；参数无误仍空 → 怀疑索引过期（embedding 未跟上新代码：未提交变更/本次新写/他人刚 push）→ **必须重新 embedding**（未索引 `index_repository`；可能过期 `detect_changes` 或重跑 `index_repository`）→ **回到图谱再查一次**。
   - **❌ 禁止"搜不到 → 跳过图谱 → 直接 read"**：那是 embedding 没跟上代码，read 到的是索引没覆盖的新代码；重索引后仍确认不存在才允许兜底直读并简述原因。
